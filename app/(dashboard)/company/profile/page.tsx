...
export default function CompanyProfilePage() {
  ...
  useEffect(() => {
    ...
    if (foundCompany) {
      ...
      // Set specialties based on company data (assuming it's stored in the company object)
      setSpecialties(foundCompany.specialties || [])
    }
  }, [user])
                ...
                <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                  <div className="space-y-4">
                    ...
                    <div className="space-y-2">
                      <Label>Специалности, за които търсите стажанти</Label>
                      <div className="flex flex-col space-y-2 mt-2">
                        <div className="flex items-center space-x-2">
                          <Checkbox
                            id="software"
                            checked={specialties.includes("software")}
                            onCheckedChange={() => handleSpecialtyChange("software")}
                          />
                          <Label htmlFor="software">Софтуерни технологии</Label>
                        </div>
                        <div className="flex items-center space-x-2">
                          <Checkbox
                            id="networks"
                            checked={specialties.includes("networks")}
                            onCheckedChange={() => handleSpecialtyChange("networks")}
                          />
                          <Label htmlFor="networks">Компютърни мрежи</Label>
                        </div>
                      </div>
                      {specialties.length === 0 && (
                        <p className="text-sm text-destructive mt-2">Моля, изберете поне една специалност.</p>
                      )}
                    </div>
